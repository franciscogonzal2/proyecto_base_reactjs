...
export class SimpleStore<StateType = any> {
	...
	// I get the current state as a stream (will always emit the current state value as the first item in the stream).
	public getState(): Observable<StateType> {
		return( this.stateSubject.asObservable() );
	}
 
	// I return the given top-level state key as a stream (will always emit the current key value as the first item in the stream).
